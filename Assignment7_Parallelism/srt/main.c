...
void * calc_colors(void * thread_x)
{
    // Cast to int
    int col = *(int *) thread_x;
    ...
    /* for every pixel */
    /* thread 1 does first col of every nthread section of the 2D array */
    /* 2D array divided vertically in sections of nthread cols */
    /* thread n works on n col of every section */
...
{
    // Needs at least 1 thread or error
    ...
    */
    ...
    /* Allocate memory for thread IDs array */
    int * thread_IDs = malloc(sizeof(int) * nthreads);
    int ret;

    /* For every thread, set the thread ID and create a thread */
    for (int i = 0; i < nthreads; i++)
    {
        thread_IDs[i] = i;
        // Passes in thread, NULL, functor to execute once thread is created, and ID # of thread which is x index
